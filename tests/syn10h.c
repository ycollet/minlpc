...
extern "C" {
 ...
 void zerdiv_(real *);
 fint syn10h_auxcom_[1] = { 6 /* nlc */ };
 fint syn10h_funcom_[326] = {
	77 /* nvar */,
	1 /* nobj */,
	112 /* ncon */,
	242 /* nzc */,
 ...
 real syn10h_boundc_[1+154+224] /* Infinity, variable bounds, constraint bounds */ = {
		1.7e308,
		0.,
		1.7e308,
		0.,
 ...
 static real syn10h_pd[46];
static real syn10h_old_x[77];
static int syn10h_xkind = -1;

 static int
syn10h_xcheck(real *x)
{
	real *x1 = syn10h_old_x, *xe = x + 77;
	errno = 0;
	if (syn10h_xkind >= 0) {
		while(*x++ == *x1++)
			if (x == xe)
 ...
 real
syn10h_feval_(fint *nobj, fint *needfg, real *x, real *g)
{
	real rv;
	fint wantfg = *needfg;
	if (syn10h_xcheck(x) && wantfg == 2)
		wantfg = 3;
 ...
 void
syn10h_ceval_(fint *needfg, real *x, real *c, real *J)
{
	real v[3], dv[3];
	real t1;
	fint wantfg = *needfg;
  ...
  /***  constraint 1  ***/
	...
	v[1] = x[2] / v[0];
	syn10h_pd[0] = 1. / v[0];
	syn10h_pd[1] = -v[1] * syn10h_pd[0];
	v[0] = x[12] + 1.e-06;
	if (v[0] == 0.) {
	zerdiv_(&v[0]);	}
	v[2] = x[0] / v[0];
	syn10h_pd[2] = 1. / v[0];
	syn10h_pd[3] = -v[2] * syn10h_pd[2];
	v[0] = v[2] + 1.;
	v[2] = log(v[0]);
	if (errno) in_trouble("log",v[0]);
	syn10h_pd[4] = 1. / v[0];
	syn10h_pd[6] = v[1] - v[2];
	syn10h_pd[5] = x[12] + 1.e-06;
	v[1] = syn10h_pd[6] * syn10h_pd[5];
	c[0] = v[1];
  ...
  /***  constraint 2  ***/
	...
	v[1] = x[3] / v[0];
	syn10h_pd[7] = 1. / v[0];
	syn10h_pd[8] = -v[1] * syn10h_pd[7];
	v[0] = x[13] + 1.e-06;
	if (v[0] == 0.) {
	zerdiv_(&v[0]);	}
	v[2] = x[1] / v[0];
	syn10h_pd[9] = 1. / v[0];
	syn10h_pd[10] = -v[2] * syn10h_pd[9];
	v[0] = v[2] + 1.;
	syn10h_pd[11] = log(v[0]);
	if (errno) in_trouble("log",v[0]);
	syn10h_pd[12] = 1. / v[0];
	v[0] = -1.2 * syn10h_pd[11];
	syn10h_pd[14] = v[1] + v[0];
	syn10h_pd[13] = x[13] + 1.e-06;
	v[1] = syn10h_pd[14] * syn10h_pd[13];
	c[1] = v[1];
  ...
  /***  constraint 3  ***/
	...
	v[1] = x[5] / v[0];
	syn10h_pd[15] = 1. / v[0];
	syn10h_pd[16] = -v[1] * syn10h_pd[15];
	v[0] = x[14] + 1.e-06;
	if (v[0] == 0.) {
	zerdiv_(&v[0]);	}
	v[2] = x[4] / v[0];
	syn10h_pd[17] = 1. / v[0];
	syn10h_pd[18] = -v[2] * syn10h_pd[17];
	v[0] = v[2] + 1.;
	syn10h_pd[19] = log(v[0]);
	if (errno) in_trouble("log",v[0]);
	syn10h_pd[20] = 1. / v[0];
	v[0] = -1.5 * syn10h_pd[19];
	syn10h_pd[22] = v[1] + v[0];
	syn10h_pd[21] = x[14] + 1.e-06;
	v[1] = syn10h_pd[22] * syn10h_pd[21];
	c[2] = v[1];
  ...
  /***  constraint 4  ***/
	...
	v[1] = x[9] / v[0];
	syn10h_pd[23] = 1. / v[0];
	syn10h_pd[24] = -v[1] * syn10h_pd[23];
	v[0] = x[15] + 1.e-06;
	if (v[0] == 0.) {
	zerdiv_(&v[0]);	}
	v[2] = x[7] / v[0];
	syn10h_pd[25] = 1. / v[0];
	syn10h_pd[26] = -v[2] * syn10h_pd[25];
	v[0] = v[2] + 1.;
	syn10h_pd[27] = log(v[0]);
	if (errno) in_trouble("log",v[0]);
	syn10h_pd[28] = 1. / v[0];
	v[0] = -1.25 * syn10h_pd[27];
	syn10h_pd[30] = v[1] + v[0];
	syn10h_pd[29] = x[15] + 1.e-06;
	v[1] = syn10h_pd[30] * syn10h_pd[29];
	c[3] = v[1];
  ...
  /***  constraint 5  ***/
	...
	v[1] = x[10] / v[0];
	syn10h_pd[31] = 1. / v[0];
	syn10h_pd[32] = -v[1] * syn10h_pd[31];
	v[0] = x[16] + 1.e-06;
	if (v[0] == 0.) {
	zerdiv_(&v[0]);	}
	v[2] = x[8] / v[0];
	syn10h_pd[33] = 1. / v[0];
	syn10h_pd[34] = -v[2] * syn10h_pd[33];
	v[0] = v[2] + 1.;
	syn10h_pd[35] = log(v[0]);
	if (errno) in_trouble("log",v[0]);
	syn10h_pd[36] = 1. / v[0];
	v[0] = -0.9 * syn10h_pd[35];
	syn10h_pd[38] = v[1] + v[0];
	syn10h_pd[37] = x[16] + 1.e-06;
	v[1] = syn10h_pd[38] * syn10h_pd[37];
	c[4] = v[1];
  ...
  /***  constraint 6  ***/
	...
	v[1] = x[11] / v[0];
	syn10h_pd[39] = 1. / v[0];
	syn10h_pd[40] = -v[1] * syn10h_pd[39];
	v[0] = x[17] + 1.e-06;
	if (v[0] == 0.) {
	zerdiv_(&v[0]);	}
	v[2] = x[6] / v[0];
	syn10h_pd[41] = 1. / v[0];
	syn10h_pd[42] = -v[2] * syn10h_pd[41];
	v[0] = v[2] + 1.;
	v[2] = log(v[0]);
	if (errno) in_trouble("log",v[0]);
	syn10h_pd[43] = 1. / v[0];
	syn10h_pd[45] = v[1] - v[2];
	syn10h_pd[44] = x[17] + 1.e-06;
	v[1] = syn10h_pd[45] * syn10h_pd[44];
	c[5] = v[1];
  ...
  /***  constraint 112  ***/
   ...
   /*** derivatives for constraint 1 ***/
	...
	dv[0] = -syn10h_pd[5];
	dv[0] *= syn10h_pd[4];
	dv[1] = dv[0]*syn10h_pd[3];
	J[0] = dv[0]*syn10h_pd[2];
	J[36] += dv[1];
	dv[2] = syn10h_pd[5]*syn10h_pd[1];
	J[6] = syn10h_pd[5]*syn10h_pd[0];
	J[36] += dv[2];
   ...
   /*** derivatives for constraint 2 ***/
	...
	dv[0] = -dv[0]*1.2;
	dv[0] *= syn10h_pd[12];
	dv[1] = dv[0]*syn10h_pd[10];
	J[3] = dv[0]*syn10h_pd[9];
	J[45] += dv[1];
	dv[2] = syn10h_pd[13]*syn10h_pd[8];
	J[9] = syn10h_pd[13]*syn10h_pd[7];
	J[45] += dv[2];
   ...
   /*** derivatives for constraint 3 ***/
	...
	dv[0] = -dv[0]*1.5;
	dv[0] *= syn10h_pd[20];
	dv[1] = dv[0]*syn10h_pd[18];
	J[12] = dv[0]*syn10h_pd[17];
	J[54] += dv[1];
	dv[2] = syn10h_pd[21]*syn10h_pd[16];
	J[15] = syn10h_pd[21]*syn10h_pd[15];
	J[54] += dv[2];
   ...
   /*** derivatives for constraint 4 ***/
	...
	dv[0] = -dv[0]*1.25;
	dv[0] *= syn10h_pd[28];
	dv[1] = dv[0]*syn10h_pd[26];
	J[21] = dv[0]*syn10h_pd[25];
	J[62] += dv[1];
	dv[2] = syn10h_pd[29]*syn10h_pd[24];
	J[27] = syn10h_pd[29]*syn10h_pd[23];
	J[62] += dv[2];
   ...
   /*** derivatives for constraint 5 ***/
	...
	dv[0] = -dv[0]*0.9;
	dv[0] *= syn10h_pd[36];
	dv[1] = dv[0]*syn10h_pd[34];
	J[24] = dv[0]*syn10h_pd[33];
	J[69] += dv[1];
	dv[2] = syn10h_pd[37]*syn10h_pd[32];
	J[30] = syn10h_pd[37]*syn10h_pd[31];
	J[69] += dv[2];
   ...
   /*** derivatives for constraint 6 ***/
	...
	dv[0] = -syn10h_pd[44];
	dv[0] *= syn10h_pd[43];
	dv[1] = dv[0]*syn10h_pd[42];
	J[18] = dv[0]*syn10h_pd[41];
	J[76] += dv[1];
	dv[2] = syn10h_pd[44]*syn10h_pd[40];
	J[33] = syn10h_pd[44]*syn10h_pd[39];
	J[76] += dv[2];
